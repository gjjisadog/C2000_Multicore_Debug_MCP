...
const baseToolDefinitions: BaseToolDefinition[] = [
  ...
  { name: "c2000_getEscalationRecommendations", title: "Get C2000 Escalation Recommendations", description: "Return deterministic, safety-aware capability and daemon-recovery recommendations for a structured workflow failure. This never opens a capability, restarts a worker, selects full access, or changes the MCP surface.", schema: getEscalationRecommendationsSchema, handlerName: "getEscalationRecommendations", inputScope: "host", targetEffect: "host-read", role: "host", family: "analytics", exposure: "default" },
  ...
  { name: "c2000_getImprovementSystemScorecard", title: "Get C2000 Improvement System Scorecard", description: "Advanced read-only meta analytics: summarize Proposal, implementation, merge, post-merge outcome, validation escape, revision, repair, and policy-regime metrics. It never returns raw history or changes policy.", schema: getImprovementSystemScorecardSchema, handlerName: "getImprovementSystemScorecard", inputScope: "host", targetEffect: "host-read", role: "host", family: "improvement", exposure: "advanced" },
  { name: "c2000_generateEngineeringPolicyRecommendations", title: "Generate C2000 Engineering Policy Recommendations", description: "Advanced deterministic governance analysis over retained Improvement outcomes. It may persist evidence-bound recommendations only after a sufficient history sample; it never edits policy, source, Skill, ToolDefinition, or Git state.", schema: generateEngineeringPolicyRecommendationsSchema, handlerName: "generateEngineeringPolicyRecommendations", inputScope: "host", targetEffect: "job-control", role: "host", family: "improvement", exposure: "advanced" },
  { name: "c2000_listEngineeringPolicyRecommendations", title: "List C2000 Engineering Policy Recommendations", description: "Advanced read-only list of evidence-bound Engineering Policy Recommendations and their human-review state. Recommendations are not executable policy changes.", schema: listEngineeringPolicyRecommendationsSchema, handlerName: "listEngineeringPolicyRecommendations", inputScope: "host", targetEffect: "host-read", role: "host", family: "improvement", exposure: "advanced" },
  { name: "c2000_getEngineeringPolicyRecommendation", title: "Get C2000 Engineering Policy Recommendation", description: "Advanced read of one meta recommendation, aggregate evidence, policy regime, protected-governance result, and conversion state without returning raw history or changing policy.", schema: getEngineeringPolicyRecommendationSchema, handlerName: "getEngineeringPolicyRecommendation", inputScope: "host", targetEffect: "host-read", role: "host", family: "improvement", exposure: "advanced" },
  { name: "c2000_reviewEngineeringPolicyRecommendation", title: "Review C2000 Engineering Policy Recommendation", description: "Advanced human governance gate to accept, reject, defer, or convert a policy recommendation into a normal ready-for-review Improvement Proposal. It never directly edits policy, runs an agent, commits, pushes, or merges.", schema: reviewEngineeringPolicyRecommendationSchema, handlerName: "reviewEngineeringPolicyRecommendation", inputScope: "host", targetEffect: "job-control", role: "host", family: "improvement", exposure: "advanced" },
  ...
  { name: "c2000_listBoards", title: "List C2000 Boards", description: "List persisted board registrations, health state, lease ownership, target-image identity generation, and quarantine evidence. A target identity of UNKNOWN requires c2000_verifyResidentImage with a valid manifest or a fresh controlled image load before symbols or observation can be trusted. If empty, call c2000_registerBoard before any daemon-routed launch.", schema: listBoardsSchema, handlerName: "listBoards", inputScope: "host", targetEffect: "host-read", role: "host", family: "host", exposure: "default" },
  { name: "c2000_registerBoard", title: "Register C2000 Board", description: "Validate a serial-bound XDS110 or XDS2xx .ccxml, persist the board registration, and start its isolated daemon worker without touching the target.", schema: registerBoardSchema, handlerName: "registerBoard", inputScope: "host", targetEffect: "job-control", role: "workflow", family: "workflow", exposure: "default" },
  { name: "c2000_recoverBoard", title: "Recover C2000 Board Worker", description: "Dry-run or restart only the daemon-owned worker for one registered board. It never kills external CCS/DSS processes and refuses restart while a board lease is active; after recovery, require a fresh session and target-image identity.", schema: recoverBoardSchema, handlerName: "recoverBoard", inputScope: "host", targetEffect: "job-control", role: "workflow", family: "workflow", exposure: "default" },
  ...
  { name: "c2000_getEradCapabilities", title: "Get F28P65x ERAD Capabilities", description: "Inspect native F28P65x ERAD ownership, occupied resources, and hardware profiling features. Firmware-instrumented CLA timing is exposed separately by c2000_readClaTaskTiming.", schema: getEradCapabilitiesSchema, handlerName: "getEradCapabilities", inputScope: "core", targetEffect: "target-read", role: "primary", family: "observability", capability: "observability.erad", coreIdentityFields: ["coreId"], responseCoreIdentityFields: ["coreId", "coreName"] },
  { name: "c2000_readClaTaskTiming", title: "Read C2000 CLA Task Timing", description: "Read a sequence-consistent firmware-instrumented CLA task timing record and report cycles and seconds using the declared timer metadata. Firmware must update the shared record at task entry and exit; this tool only reads target memory.", schema: readClaTaskTimingSchema, handlerName: "readClaTaskTiming", inputScope: "core", targetEffect: "target-read", role: "primary", family: "observability", capability: "observability.erad", coreIdentityFields: ["coreId"], responseCoreIdentityFields: ["coreId", "coreName"] },
  ...
  { name: "c2000_getAcceptanceReadiness", title: "Get C2000 Acceptance Readiness", description: "Return a read-only hardware acceptance readiness report that combines ccxml, CPU program discovery, XDS110/XDS2xx preflight, debug ownership, and debug boundary checks.", schema: acceptanceReadinessSchema, handlerName: "getAcceptanceReadiness", inputScope: "host", targetEffect: "host-read", role: "host", family: "host" },
  ...
  { name: "c2000_getTargetState", title: "Get C2000 Target State", description: "Read connection state and run state for one explicit core without reading PC; use c2000_resolvePc for an explicit PC read.", schema: sessionCoreSchema, handlerName: "getTargetState", inputScope: "core", targetEffect: "target-read", role: "primary", family: "read", capability: "debug.manual", coreIdentityFields: ["coreId"], responseCoreIdentityFields: [...singleCoreResponseIdentity] },
  ...
  { name: "c2000_loadSymbols", title: "Load C2000 Symbols Only", description: "Load debug symbols from a .out file into one explicit core session without erasing, programming, or writing target memory. Use for an image already resident in Flash; the daemon requires a known target-image identity and matching SHA-256 for the requested core.", schema: loadSymbolsSchema, handlerName: "loadSymbols", inputScope: "core", targetEffect: "symbol-load", role: "primary", family: "program", capability: "debug.program", coreIdentityFields: ["coreId"], responseCoreIdentityFields: [...singleCoreResponseIdentity] },
  ...
  { name: "c2000_runCores", title: "Run C2000 Cores", description: "Run multiple cores by explicit coreIds and return each post-command core state; a core observed Halted is reported as a failed run result.", schema: batchCoresSchema, handlerName: "runCores", inputScope: "batch", targetEffect: "execution-control", role: "primary", family: "execution", capability: "debug.manual", coreIdentityFields: ["coreIds[]"], responseCoreIdentityFields: [...batchCoreResponseIdentity] },
  { name: "c2000_getMulticoreSnapshot", title: "Get C2000 Multicore Snapshot", description: "Read state and loaded program for explicit coreIds, defaulting to every core in a session, and explicitly read PC for connected cores. Target reads can perturb real-time execution on CCS; avoid high-rate polling during live UART/IPC timing checks.", schema: multicoreSnapshotSchema, handlerName: "getMulticoreSnapshot", inputScope: "session", targetEffect: "target-read", role: "primary", family: "read", exposure: "default", coreIdentityFields: ["coreIds[]"], responseCoreIdentityFields: [...snapshotCoreResponseIdentity] },
  { name: "c2000_verifyResidentImage", title: "Verify C2000 Resident Image", description: "Attach to explicit cores when requested and verify a manifest-bound resident-image marker using raw memory. This never programs Flash, loads symbols, resets, runs, or writes target memory; only a complete match promotes the board identity from UNKNOWN to KNOWN for the requested cores.", schema: verifyResidentImageSchema, handlerName: "verifyResidentImage", inputScope: "session", targetEffect: "target-read", role: "primary", family: "read", exposure: "default", coreIdentityFields: ["checks[].coreId"], responseCoreIdentityFields: ["checks[].coreId", "checks[].coreName"] },
  ...
  { name: "c2000_resolvePc", title: "Resolve C2000 PC", description: "Explicitly read and resolve the current PC for one core, returning partial data when source mapping is unavailable; this is separate from c2000_getTargetState.", schema: sessionCoreSchema, handlerName: "resolvePc", inputScope: "core", targetEffect: "target-read", role: "primary", family: "read", capability: "debug.manual", coreIdentityFields: ["coreId"], responseCoreIdentityFields: [...coreOnlyResponseIdentity] },
  ...
  { name: "c2000_reloadResetRunToMain", title: "Reload Reset Run C2000 Core", description: "Reload one explicit core, perform a core-local reset/run, and optionally verify that its PC enters the supplied linker-map or entry-address application range. Without entry evidence, the response honestly reports that the current adapter has no breakpoint/runToSymbol API.", schema: reloadResetRunToMainSchema, handlerName: "reloadResetRunToMain", inputScope: "core", targetEffect: "launch-workflow", role: "workflow", family: "workflow", coreIdentityFields: ["coreId"], responseCoreIdentityFields: [...singleCoreResponseIdentity] },
  { name: "c2000_launchAndRunIpcAcceptance", title: "Launch And Run C2000 IPC Acceptance", description: "Preferred task-level one-shot after c2000_listBoards confirms a registered worker: create and connect CPU1/CPU2 (standard coreIds 0 and 2), then run full IPC acceptance. The server validates the load/run contract and real-map IPC symbols before target mutation. programPreparation=symbols-only loads matching .out symbols without programming or verifying resident Flash. For RAM builds that initialize ownership from CPU1, use loadSequence.mode=cpu1-run-before-cpu2. For firmware-owned CPU2 release, use runSequence.runMode=cpu1_boots_cpu2: load both images, disconnect CPU2, restart/run CPU1 to a linker-map or cpu1EntryAddress range, reconnect CPU2 only after entry evidence, then check IPC. A failed launch closes its new session by default; set cleanupOnFailure=false only for deliberate interactive recovery.", schema: launchAndRunIpcAcceptanceSchema, handlerName: "launchAndRunIpcAcceptance", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...launchAndRunIpcAcceptanceResponseIdentity] },
  { name: "c2000_runIpcAcceptance", title: "Run C2000 IPC Acceptance Workflow", description: "Preferred task-level workflow when a session already exists: full F28P65x CPU1/CPU2 IPC acceptance in one server-side call. The server validates the load/run contract and real-map IPC symbols before target mutation; programPreparation=symbols-only loads matching .out symbols without programming or verifying resident Flash. In a durable symbols-only flow, preStartupSafetyGuard is evaluated after symbols load and before reset/run. When both images are programmed into Flash, use startupPreset=f28p65x-paired-flash: both Flash images are programmed while every application core stays halted and no core is started inside that boundary. Use this instead of launchMulticoreDebug followed by generic runCores for an IPC handshake. For firmware-owned CPU2 release, runSequence.runMode=cpu1_boots_cpu2 enforces load → CPU2 disconnect → CPU1-only restart/run and bounded PC entry confirmation; IPC readiness is skipped when APPLICATION_ENTRY_NOT_REACHED is reported.", schema: runIpcAcceptanceSchema, handlerName: "runIpcAcceptance", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...workflowIpcAcceptanceResponseIdentity] },
  { name: "c2000_runResidentIpcDebug", title: "Run Resident C2000 IPC Debug", description: "Preferred one-call path for a connected session whose F28P65x Flash image is already resident: reuse the MCP-loaded CPU1/CPU2 .out pair when paths are omitted, infer adjacent maps, and attach without reset/run by default. mode=restart-and-diagnose opts into the legacy bounded CPU2 handoff. It never programs Flash or writes target memory. A fresh lease uses operator-confirmed resident identity by default, compares requested .out hashes to the last MCP evidence when available, and requires residentImageManifests for read-back proof. Evidence bundles are off by default; when enabled without outputDir, MCP chooses an allowed runtime write root.", schema: runResidentIpcDebugSchema, handlerName: "runResidentIpcDebug", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...workflowIpcAcceptanceResponseIdentity] },
  { name: "c2000_launchResidentIpcDebug", title: "Launch Resident C2000 IPC Debug", description: "Preferred no-session path for an already-programmed F28P65x Flash image: select the registered board, close an older MCP-created resident session when replaceExistingResidentSession=true, recover an expired MCP lease only when the current worker identity matches, create/connect CPU1/CPU2, and attach without reset/run by default. Probe contention fails fast (15s by default) and removes the queued request. mode=restart-and-diagnose opts into the legacy bounded handoff. It never programs Flash or writes target memory. With one registered board, boardId may be omitted; with multiple boards, provide boardId. A fresh lease uses operator-confirmed resident identity by default, compares requested .out hashes to the last MCP evidence when available, and requires residentImageManifests for read-back proof.", schema: launchResidentIpcDebugSchema, handlerName: "launchResidentIpcDebug", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...launchAndRunIpcAcceptanceResponseIdentity] },
...
export function getToolSurfaceGuide(
  ...
  const surfaceGuidance = surface === "agent"
    ...
    : [
        "Prefer one workflow tool (c2000_launchResidentIpcDebug, c2000_launchAndRunIpcAcceptance, c2000_runResidentIpcDebug, c2000_runIpcAcceptance, c2000_runBootHandoffDiagnosis, c2000_runReloadAndDiagnose, c2000_runFullDebugBundle) over long atomic chains.",
  ...
  return {
    ...
    guidance: [
      ...
      ...(surface === "agent"
        ? ["For resident Flash use c2000_launchResidentIpcDebug when no session exists, or c2000_runResidentIpcDebug for an existing session; for programming or a custom contract use c2000_runIpcAcceptance or c2000_launchAndRunIpcAcceptance. The task-level workflow performs the boot-handoff contract.", "Capability sessions are temporary and reason-bound. Open observability.dlog, observability.erad, observability.variables, debug.manual, debug.program, or debug.wait only when the task requires it; the active safety profile still filters every member tool."]
        : ["For resident Flash use c2000_launchResidentIpcDebug when no session exists, or c2000_runResidentIpcDebug for an existing session; for programming or a custom contract use c2000_runIpcAcceptance or c2000_launchAndRunIpcAcceptance. LaunchMulticoreDebug followed by generic runCores does not perform the boot-handoff contract."]),
      ...(surface === "agent"
        ? []
...
function decorateDefinition(definition: BaseToolDefinition): ToolDefinition {
  ...
  const residentWorkflowConfirmation = definition.targetEffect === "launch-workflow"
    ...
    && !effects.includes("ram-ownership-change")
    && (definition.name === "c2000_runResidentIpcDebug" || definition.name === "c2000_launchResidentIpcDebug");
    ...
    approvalClass: readOnlyHint
      ...
      : definition.targetEffect === "session-lifecycle" ? "session-lifecycle"
        : effects.includes("program-load") ? "program-load"
          : residentWorkflowConfirmation ? "workflow-confirmation"
...
function effectsFor(name: string, targetEffect: ToolTargetEffect): ToolEffect[] {
  ...
  if (name === "c2000_runResidentIpcDebug") return ["target-halt", "target-reset", "symbol-load", "target-run", "target-read", "target-disconnect", "target-connect"];
  if (name === "c2000_launchResidentIpcDebug") return ["session-create", "target-halt", "target-reset", "symbol-load", "target-run", "target-read", "target-disconnect", "target-connect"];
