...
const baseToolDefinitions: BaseToolDefinition[] = [
  ...
  { name: "c2000_reloadResetRunToMain", title: "Reload Reset Run C2000 Core", description: "Reload one explicit core, reset it, run it, and optionally verify that its PC enters an application linker-map range; true breakpoint run-to-main remains unsupported by the current adapter.", schema: reloadResetRunToMainSchema, handlerName: "reloadResetRunToMain", inputScope: "core", targetEffect: "launch-workflow", role: "workflow", family: "workflow", coreIdentityFields: ["coreId"], responseCoreIdentityFields: [...singleCoreResponseIdentity] },
  { name: "c2000_launchAndRunIpcAcceptance", title: "Launch And Run C2000 IPC Acceptance", description: "Preferred task-level one-shot after c2000_listBoards confirms a registered worker: create and connect CPU1/CPU2 (standard coreIds 0 and 2), then run full IPC acceptance. The server validates the load/run contract and real-map IPC symbols before target mutation. For firmware-owned CPU2 release, it requires a bounded CPU1 application-entry check before reconnecting CPU2; failed launches clean up by default and can preserve an interactive session only when explicitly requested.", schema: launchAndRunIpcAcceptanceSchema, handlerName: "launchAndRunIpcAcceptance", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...launchAndRunIpcAcceptanceResponseIdentity] },
  { name: "c2000_runIpcAcceptance", title: "Run C2000 IPC Acceptance Workflow", description: "Preferred task-level workflow when a session already exists: full F28P65x CPU1/CPU2 IPC acceptance in one server-side call. The server validates the load/run contract and real-map IPC symbols before target mutation; for firmware-owned CPU2 release, it verifies CPU1 enters a bounded application linker-map range before reconnecting CPU2 or polling IPC readiness. Use this instead of launchMulticoreDebug followed by generic runCores for an IPC handshake.", schema: runIpcAcceptanceSchema, handlerName: "runIpcAcceptance", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...workflowIpcAcceptanceResponseIdentity] },
