...
const baseToolDefinitions: BaseToolDefinition[] = [
  ...
  { name: "c2000_runResidentIpcDebug", title: "Run Resident C2000 IPC Debug", description: "Preferred one-call path for a connected session whose F28P65x Flash image is already resident: load CPU1/CPU2 symbols only, use firmware-owned CPU2 release by default, and capture bounded IPC/boot diagnosis. It never programs Flash or writes target memory. If resident identity is UNKNOWN, the workflow stops with the manifest or exact-pair next action instead of guessing. Evidence bundles are off by default; when enabled without outputDir, MCP chooses an allowed runtime write root.", schema: runResidentIpcDebugSchema, handlerName: "runResidentIpcDebug", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...workflowIpcAcceptanceResponseIdentity] },
...
export function getToolSurfaceGuide(
  ...
  const surfaceGuidance = surface === "agent"
    ...
    : [
        "Prefer one workflow tool (c2000_launchAndRunIpcAcceptance, c2000_runResidentIpcDebug, c2000_runIpcAcceptance, c2000_runBootHandoffDiagnosis, c2000_runReloadAndDiagnose, c2000_runFullDebugBundle) over long atomic chains.",
  ...
  return {
    ...
    guidance: [
      ...
      ...(surface === "agent"
        ? ["For resident Flash use c2000_runResidentIpcDebug; for programming or a custom contract use c2000_runIpcAcceptance or c2000_launchAndRunIpcAcceptance. The task-level workflow performs the boot-handoff contract.", "Capability sessions are temporary and reason-bound. Open observability.dlog, observability.erad, observability.variables, debug.manual, debug.program, or debug.wait only when the task requires it; the active safety profile still filters every member tool."]
        : ["For resident Flash use c2000_runResidentIpcDebug; for programming or a custom contract use c2000_runIpcAcceptance or c2000_launchAndRunIpcAcceptance. LaunchMulticoreDebug followed by generic runCores does not perform the boot-handoff contract."]),
      ...(surface === "agent"
        ? []
      ...
      ...(surface === "agent"
        ? ["If firmware is already resident in Flash, use c2000_runResidentIpcDebug; it is the resident-image symbol path and never substitutes a program load."]
