...
const baseToolDefinitions: BaseToolDefinition[] = [
  ...
  { name: "c2000_runResidentIpcDebug", title: "Run Resident C2000 IPC Debug", description: "Preferred one-call path for a connected session whose F28P65x Flash image is already resident: reuse the MCP-loaded CPU1/CPU2 .out pair when paths are omitted, infer adjacent maps, optionally verify residentImageManifests before symbols, then use firmware-owned CPU2 release by default and capture bounded IPC/boot diagnosis. It never programs Flash or writes target memory. If identity is UNKNOWN and no manifest is supplied, the workflow stops with the exact-pair or manifest next action instead of guessing. Evidence bundles are off by default; when enabled without outputDir, MCP chooses an allowed runtime write root.", schema: runResidentIpcDebugSchema, handlerName: "runResidentIpcDebug", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...workflowIpcAcceptanceResponseIdentity] },
  { name: "c2000_launchResidentIpcDebug", title: "Launch Resident C2000 IPC Debug", description: "Preferred no-session path for an already-programmed F28P65x Flash image: select the registered board, acquire its lease, create and connect CPU1/CPU2, optionally verify residentImageManifests before symbols, then run bounded IPC/boot diagnosis. It never programs Flash or writes target memory. With one registered board, boardId may be omitted; with multiple boards, provide boardId. The default residentIdentityPolicy=require-known requires daemon-verified identity; supplying a manifest lets this call establish UNKNOWN identity through a read-only marker check. Use residentIdentityPolicy=operator-confirmed only for an explicit operator confirmation that Flash was not changed outside MCP.", schema: launchResidentIpcDebugSchema, handlerName: "launchResidentIpcDebug", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", exposure: "default", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...launchAndRunIpcAcceptanceResponseIdentity] },
