...
const baseToolDefinitions: Array<Omit<ToolDefinition, "effects" | "annotations" | "approvalClass">> = [
  ...
  { name: "c2000_runIpcAcceptance", title: "Run C2000 IPC Acceptance Workflow", description: "Preferred when session already exists: full F28P65x CPU1/CPU2 IPC acceptance in one server-side call. Supports CPU1 pre-run before CPU2 RAM load through loadSequence; runMode=cpu1_boots_cpu2 disconnects CPU2 during the firmware-owned handoff and reconnects it before diagnosis.", schema: runIpcAcceptanceSchema, handlerName: "runIpcAcceptance", inputScope: "launch", targetEffect: "launch-workflow", role: "workflow", family: "workflow", coreIdentityFields: ["cpu1CoreId", "cpu2CoreId", "ipcReadyExpressions[].coreId"], responseCoreIdentityFields: [...workflowIpcAcceptanceResponseIdentity] },
