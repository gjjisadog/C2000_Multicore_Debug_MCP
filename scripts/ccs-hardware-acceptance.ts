...
import { hasBlockingDebugProcesses, formatDebugProcessOwners, runHardwarePreflight } from "../src/hardware/preflight.js";
...
const cpu1Program = requiredProgram("C2000_CPU1_OUT", programDiscovery.cpu1);
const cpu2Program = requiredProgram("C2000_CPU2_OUT", programDiscovery.cpu2);
...
const dssTimeoutMs = Number.parseInt(process.env.C2000_MCP_DSS_TIMEOUT_MS ?? "300000", 10);
...
const contracts = await tools.getToolContracts({});
assertSuccess("c2000_getToolContracts", contracts);
assertToolContracts(contracts);
const debugBoundary = await tools.getDebugBoundary({});
...
const acceptanceEvidence = await tools.getAcceptanceEvidence({});
...
try {
  const created = await tools.createDebugSession({
    sessionName: "f28p65x-hardware-acceptance",
    ccxmlPath,
    coreMap
  });
  assertSuccess("c2000_createDebugSession", created, { preflight });
  const createdSessionId = String(created.sessionId);
  sessionId = createdSessionId;

  const topology = await tools.getSessionTopology({ sessionId: createdSessionId });
  assertSuccess("c2000_getSessionTopology", topology, { preflight });
  assertSessionTopology(topology);

  const connect = await tools.connectCores({ sessionId: createdSessionId, coreIds: [0, 2] });
  assertSuccess("c2000_connectCores", connect, { preflight });
  assertBatchCoreResults("c2000_connectCores", connect, [0, 2]);

  const load = await tools.loadPrograms({
    sessionId: createdSessionId,
    programs: [
  ...
  const initialSnapshot = await tools.getMulticoreSnapshot({ sessionId: createdSessionId });
  ...
  result.debugBoundary = debugBoundary;
  ...
  if (runIsolation) {
    const isolation = await tools.verifyRunPauseIsolation({ sessionId: createdSessionId });
  ...
  if (runLaunch) {
    const closeMain = await tools.closeDebugSession({ sessionId: createdSessionId });
    assertSuccess("c2000_closeDebugSession(main-before-launch)", closeMain, { preflight });
    sessionId = undefined;

    const launch = await tools.launchMulticoreDebug({
      sessionName: "f28p65x-hardware-launch-acceptance",
      ccxmlPath,
      programDiscovery: {
        enabled: true,
        cpu1Program,
  ...
  if (sessionId) {
    try {
      await tools.closeDebugSession({ sessionId });
    } catch (cleanupError) {
      if (primaryError) {
  ...
  if (launchSessionId) {
    try {
      await tools.closeDebugSession({ sessionId: launchSessionId });
    } catch (cleanupError) {
      if (primaryError) {
